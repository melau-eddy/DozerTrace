...
void setup() {
	...
	Serial.flush();
	delay(500);
  if (!SPIFFS.begin(true)) {  // Format SPIFFS if it fails
...
#ifdef ESP32BUILD
  pinMode(16,OUTPUT);
  digitalWrite(4, LOW);    // set GPIO16 low to reset OLED
  delay(50);
  digitalWrite(4, HIGH); // while OLED is running, must set GPIO16 in high、
#endif
  display.init();
